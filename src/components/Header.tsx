...
export const Header: React.FC<HeaderProps> = ({
  ...
  const pushActivo = webPushConfig.habilitado && permisoNavegador === 'granted';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2.5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {/* Botón de Seguridad & Perfil de Usuario por Gerencia (Opción A) */}
            {usuarioActivo && onAbrirSelectorPerfil && (
              <button
                id="btn-usuario-activo-header"
                type="button"
                onClick={onAbrirSelectorPerfil}
                className={`inline-flex items-center gap-2 px-2.5 py-1 text-xs font-bold rounded-lg border transition-all hover:scale-[1.02] shadow-2xs cursor-pointer group ${usuarioActivo.colorTema.bgLight} ${usuarioActivo.colorTema.border}`}
                title={`Sesión activa: ${usuarioActivo.nombre} (${usuarioActivo.titular}). Haga clic para cambiar de gerencia o cambiar su PIN.`}
              >
                <div className={`w-5 h-5 rounded-md flex items-center justify-center text-white ${usuarioActivo.colorTema.badgeBg} shadow-2xs`}>
                  {usuarioActivo.id === 'gerencia-general' && <Building2 className="w-3 h-3" />}
                  {usuarioActivo.id === 'gerencia-academica' && <GraduationCap className="w-3 h-3" />}
                  {usuarioActivo.id === 'gerencia-comercializacion' && <Megaphone className="w-3 h-3" />}
                  {usuarioActivo.id === 'auditor-interno' && <Scale className="w-3 h-3" />}
                </div>
                <div className="flex flex-col items-start text-left leading-tight">
                  <span className="text-[9px] uppercase tracking-wider text-slate-500 font-extrabold flex items-center gap-1">
                    <span>Sesión</span>
                    <KeyRound className="w-2.5 h-2.5 text-slate-400 group-hover:text-indigo-600 transition-colors" />
                  </span>
                  <span className={`text-[11px] font-black ${usuarioActivo.colorTema.accentColor}`}>
                    {usuarioActivo.nombre}
                  </span>
                </div>
                <span className="hidden sm:inline-block px-1.5 py-0.5 rounded text-[9px] font-semibold bg-white/80 border border-slate-200 text-slate-600 group-hover:bg-white group-hover:text-slate-900 transition-colors ml-0.5">
                  Cambiar PIN
                </span>
              </button>
            )}
            ...
            {/* Botón Web Push de Navegador (Segundo Plano) */}
            {onAbrirWebPushModal && (
              <button
                id="btn-webpush-header"
                onClick={onAbrirWebPushModal}
                className={`relative inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-lg transition-colors shadow-2xs border ${
                  pushActivo
                    ? 'bg-emerald-50/90 hover:bg-emerald-100 text-emerald-950 border-emerald-300'
                    : 'bg-white hover:bg-slate-50 text-slate-700 border-slate-200'
                }`}
                title="Configurar Notificaciones Web Push de Navegador (Alertas en segundo plano)"
              >
                <BellRing className={`w-3.5 h-3.5 ${pushActivo ? 'text-emerald-600' : 'text-slate-400'}`} />
                <span className="hidden sm:inline">Web Push</span>
                <span className={`w-2 h-2 rounded-full ${pushActivo ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`} />
              </button>
            )}
            ...
            <div className="relative group">
              ...
              <div className="absolute right-0 mt-1 w-64 bg-white rounded-xl shadow-xl border border-slate-200 py-1.5 hidden group-hover:block z-50 animate-fadeIn">
                ...
                >
                  ...
                  <div>
                    <div className="font-bold text-slate-800">1. Gerencia Académica</div>
                    <div className="text-[10px] text-slate-500">Sílabos Oficiales, Carga Horaria, Docentes</div>
                  </div>
                </button>
                ...
                <button
                  ...
                  className="w-full text-left px-3 py-2 text-xs text-slate-700 hover:bg-purple-50 hover:text-purple-900 flex items-center gap-2.5 transition-colors"
                >
                  <div className="w-6 h-6 rounded-md bg-purple-100 text-purple-700 flex items-center justify-center shrink-0">
                    <Building2 className="w-3.5 h-3.5" />
                  </div>
                  <div>
                    <div className="font-bold text-slate-800">2. Gerencia General</div>
                    <div className="text-[10px] text-slate-500">Cola Revisión, P&L, Aprobación/Rechazo</div>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    <div className="font-bold text-slate-800">3. Gerencia Comercial</div>
                    <div className="text-[10px] text-slate-500">Punto de Equilibrio, Matrícula, Tarifas ISV</div>
                  </div>
                </button>
      ...
      <div className="bg-slate-50/90 border-t border-slate-200/80 px-4 sm:px-6 lg:px-8 py-1.5 overflow-x-auto no-scrollbar">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3 text-xs">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            {/* 1. Gerencia Académica (Paso 1: Elabora el Proyecto y Sílabos) */}
            <button
              ...
              title={`Paso 1: Elaboración Curricular y Sílabos • ${CREDENCIALES_GERENCIAS.academica.lider} (${CREDENCIALES_GERENCIAS.academica.correo})`}
            >
              ...
              <div className="flex flex-col items-start leading-tight">
                <span className="flex items-center gap-1">
                  ...
                  {usuarioActivo?.id === 'gerencia-academica' && (
                    <span className="text-[8px] bg-blue-200 text-blue-950 px-1 py-0.2 rounded font-black">
                      Tu Sesión
                    </span>
                  )}
                </span>
            ...
            {/* 2. Gerencia General (Paso 2: Revisión, Dictamen & Aprobación) */}
            <button
              ...
              title={`Paso 2: Revisión Ejecutiva, Aprobación/Rechazo de Sílabos • ${CREDENCIALES_GERENCIAS.administracion.lider} (${CREDENCIALES_GERENCIAS.administracion.correo})`}
            >
              ...
              <div className="flex flex-col items-start leading-tight">
                <span className="flex items-center gap-1">
                  <span className="text-[9px] px-1 py-0.2 rounded bg-purple-100 text-purple-900 font-extrabold">2</span>
                  <span>G. General</span>
                  {usuarioActivo?.id === 'gerencia-general' && (
                    <span className="text-[8px] bg-purple-300 text-purple-950 px-1 py-0.2 rounded font-black">
                      Tu Sesión
                    </span>
                  )}
                </span>
            ...
            {/* 3. Gerencia de Comercialización (Paso 3: Venta y Matrícula tras Aprobación) */}
            <button
              ...
              title={`Paso 3: Comercialización y Matrícula • ${CREDENCIALES_GERENCIAS.comercial.lider} (${CREDENCIALES_GERENCIAS.comercial.correo})`}
            >
              <Megaphone className={`w-3.5 h-3.5 ${vistaActual === 'gerencia-comercializacion' ? 'text-emerald-200' : 'text-emerald-600'}`} />
              <div className="flex flex-col items-start leading-tight">
                <span className="flex items-center gap-1">
                  <span className="text-[9px] px-1 py-0.2 rounded bg-emerald-100/80 text-emerald-900 font-extrabold">3</span>
                  <span>G. Comercial</span>
                  {usuarioActivo?.id === 'gerencia-comercializacion' && (
                    <span className="text-[8px] bg-emerald-200 text-emerald-950 px-1 py-0.2 rounded font-black">
                      Tu Sesión
                    </span>
                  )}
                </span>
            ...
            {/* 4. Auditoría Interna (si está activo el perfil) */}
            {usuarioActivo?.id === 'auditor-interno' && (
              <>
                <span className="text-slate-300 font-bold hidden sm:inline">|</span>
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-amber-600 text-white shadow-xs">
                  <Scale className="w-3.5 h-3.5 text-amber-200" />
                  <div className="flex flex-col items-start leading-tight">
                    <span className="flex items-center gap-1">
                      <span>Auditoría & SAR</span>
                      <span className="text-[8px] bg-amber-200 text-amber-950 px-1 py-0.2 rounded font-black">
                        Tu Sesión
                      </span>
                    </span>
                    <span className="hidden xl:inline text-[9px] font-medium opacity-90">
                      Fiscalización Solo Lectura
                    </span>
                  </div>
                </div>
              </>

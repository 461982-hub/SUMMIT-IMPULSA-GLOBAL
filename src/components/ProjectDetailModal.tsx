...
import { enviarReportePorEmail, CORREOS_GERENCIAS_DEFAULT } from '../utils/gmailUtils';
...
export const ProjectDetailModal: React.FC<ProjectDetailModalProps> = ({
  ...
  const [modalFormsAbierto, setModalFormsAbierto] = useState<boolean>(false);

  // Estados para envío de reporte vía Gmail API
  const [modalEmailAbierto, setModalEmailAbierto] = useState<boolean>(false);
  const [enviandoEmail, setEnviandoEmail] = useState<boolean>(false);
  const [destinatariosEmail, setDestinatariosEmail] = useState<string[]>(CORREOS_GERENCIAS_DEFAULT);
  const [notaEmail, setNotaEmail] = useState<string>('');
  const [resultadoEnvioEmail, setResultadoEnvioEmail] = useState<{ exito: boolean; mensaje: string } | null>(null);

  const handleToggleDestinatario = (correo: string) => {
    setDestinatariosEmail(prev => 
  ...
  const handleEnviarEmail = async () => {
    if (!proyecto || destinatariosEmail.length === 0) return;
    setEnviandoEmail(true);
    setResultadoEnvioEmail(null);

    try {
      const res = await enviarReportePorEmail({
        proyecto,
        moneda,
        destinatarios: destinatariosEmail,
        comentarioAdicional: notaEmail,
      });

      if (res.success) {
        setResultadoEnvioEmail({
          exito: true,
          mensaje: `Reporte enviado exitosamente a ${res.destinatarios?.length || destinatariosEmail.length} gerencia(s) vía Gmail API (ID: ${res.messageId || 'OK'}).`,
        });

        if (onAgregarHistorial) {
          onAgregarHistorial(proyecto.id, {
            id: `hist-email-${Date.now()}`,
            fecha: new Date().toISOString().slice(0, 10),
            hora: new Date().toLocaleTimeString('es-HN', { hour: '2-digit', minute: '2-digit', hour12: true }),
            usuario: 'Sistema Gmail OAuth2',
            rol: 'Notificación Multi-Gerencia',
            campoModificado: 'Notificación por Correo',
            valorAnterior: '-',
            valorNuevo: `Enviado a: ${destinatariosEmail.join(', ')}`,
            justificacion: 'Envío oficial de reporte de rentabilidad vía Gmail API OAuth2.',
          });
        }
      } else {
        setResultadoEnvioEmail({
          exito: false,
          mensaje: res.error || 'No se pudo enviar el correo vía Gmail.',
        });
      }
    } catch (err: any) {
      setResultadoEnvioEmail({
        exito: false,
        mensaje: err?.message || 'Error al autenticar o enviar correo con Gmail.',
      });
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4">
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {proyecto.codigoSilaboOrigen && (
                  <span className="px-2 py-0.5 rounded text-xs font-mono font-bold bg-indigo-100 text-indigo-900 border border-indigo-200 flex items-center gap-1" title={`Vinculado al Sílabo Base: ${proyecto.nombreSilaboOrigen || ''}`}>
                    <BookOpen className="w-3 h-3 text-indigo-600" />
                    Sílabo Base: {proyecto.codigoSilaboOrigen}
                  </span>
                )}
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              id="btn-modal-exportar-sheets"
              type="button"
              onClick={() => setModalSheetsAbierto(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-teal-800 bg-teal-50 hover:bg-teal-100 rounded-lg transition-colors border border-teal-300 shadow-xs cursor-pointer"
              title="Exportar proyecto a Google Sheets en tiempo real"
            >
            ...
            <button
              id="btn-modal-google-forms"
              type="button"
              onClick={() => setModalFormsAbierto(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-purple-800 bg-purple-50 hover:bg-purple-100 rounded-lg transition-colors border border-purple-300 shadow-xs cursor-pointer"
              title="Generar formulario de pre-matrícula o evaluación docente en Google Forms"
            >
            ...
            <button
              id="btn-modal-enviar-email"
              type="button"
              onClick={() => {
                setResultadoEnvioEmail(null);
                setModalEmailAbierto(true);
              }}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-red-800 bg-red-50 hover:bg-red-100 rounded-lg transition-colors border border-red-200 shadow-xs cursor-pointer"
              title="Enviar reporte del proyecto a los correos de las gerencias vía Gmail API"
            >
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          {tabActiva === 'historial' ? (
            ...
            <>
              {/* Barra de Trazabilidad y Estado del Flujo Secuencial (Académica -> Comercial -> General con Rebaja POA 2026) */}
              <div className="bg-slate-900 text-white p-4 rounded-2xl border border-slate-800 shadow-md">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3 pb-2 border-b border-slate-800">
                  <span className="text-[11px] font-extrabold uppercase tracking-wider text-slate-300 flex items-center gap-1.5">
                    ...
                    Flujo Institucional Secuencial & Rebaja POA 2026
                  </span>
                  <span className="text-[11px] font-mono text-emerald-400 font-bold">
                    {proyecto.aprobacionFinalGerenciaGeneral 
                      ? '✓ APROBADO & REBAJADO DEL POA 2026' 
                      : proyecto.autorizacionComercial 
                      ? '⏳ EN REVISIÓN POR GERENCIA GENERAL'
                      : '🚀 EN PROCESO DE COMERCIALIZACIÓN & MATRÍCULA'}
                  </span>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {/* Paso 1: Gerencia Académica */}
                  <div className="bg-slate-800/80 rounded-xl p-2.5 border border-emerald-500/40 flex items-start gap-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-[11px] font-bold text-white flex items-center justify-between">
                        ...
                        <span className="text-[9px] text-emerald-400 font-semibold">Guardado</span>
                      </div>
                      <p className="text-[10px] text-slate-300 mt-0.5 leading-tight">
                        Estructura académica validada ({proyecto.horasClase}h, {proyecto.diasClase || 'días de clase'}).
                      </p>
                    </div>
                  </div>

                  {/* Paso 2: Gerencia Comercial */}
                  <div className={`rounded-xl p-2.5 border flex items-start gap-2.5 ${
                    proyecto.autorizacionComercial 
                      ? 'bg-slate-800/80 border-emerald-500/40 text-white' 
                      : 'bg-emerald-950/60 border-emerald-400/60 text-emerald-100 ring-1 ring-emerald-400/30'
                  }`}>
                    <div className={`w-6 h-6 rounded-full font-black text-xs flex items-center justify-center shrink-0 mt-0.5 ${
                      proyecto.autorizacionComercial 
                        ? 'bg-emerald-500 text-slate-950' 
                        : 'bg-amber-400 text-slate-950 animate-pulse'
                    }`}>
                      {proyecto.autorizacionComercial ? '✓' : '2'}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="text-[11px] font-bold flex items-center justify-between">
                        <span>2. Gerencia Comercial</span>
                        <span className={`text-[9px] font-semibold ${proyecto.autorizacionComercial ? 'text-emerald-400' : 'text-amber-300'}`}>
                          {proyecto.autorizacionComercial ? 'Autorizado' : 'Venta & Matrícula'}
                        </span>
                      </div>
                      <p className="text-[10px] text-slate-300 mt-0.5 leading-tight">
                        Venta y matrícula ({proyecto.alumnosFinal || proyecto.alumnosProyectados} inscritos).
                      </p>
                    </div>
                  </div>
                  ...
                  <div className={`rounded-xl p-2.5 border flex items-start gap-2.5 ${
                    proyecto.aprobacionFinalGerenciaGeneral 
                      ...
                      : proyecto.autorizacionComercial 
                      ? 'bg-purple-950/60 border-purple-400/60 text-purple-100 ring-1 ring-purple-400/30'
                    ...
                    <div className={`w-6 h-6 rounded-full font-black text-xs flex items-center justify-center shrink-0 mt-0.5 ${
                      ...
                      {proyecto.aprobacionFinalGerenciaGeneral ? '✓' : '3'}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="text-[11px] font-bold flex items-center justify-between">
                        <span>3. Gerencia General</span>
                        <span className={`text-[9px] font-semibold ${
                          proyecto.aprobacionFinalGerenciaGeneral ? 'text-emerald-400' : 'text-slate-400'
                        }`}>
                          {proyecto.aprobacionFinalGerenciaGeneral ? 'Rebajado POA' : 'Pendiente'}
                        </span>
                      </div>
                      <p className="text-[10px] text-slate-300 mt-0.5 leading-tight">
                        {proyecto.aprobacionFinalGerenciaGeneral 
                          ? `Rebajado del POA 2026: ${formatearMoneda(proyecto.montoFacturacionAprobadaHNL || proyecto.ingresoRealTotal || 0, moneda)}` 
                          : 'Revisión final, aprobación y rebaja del POA 2026.'}
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <div className="px-6 py-3 border-t border-slate-200 bg-slate-50 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <button
              id="btn-footer-enviar-email"
              type="button"
              onClick={() => {
                setResultadoEnvioEmail(null);
                setModalEmailAbierto(true);
              }}
              className="flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-bold text-slate-800 hover:text-red-700 bg-white hover:bg-red-50 border border-slate-300 hover:border-red-300 rounded-lg transition-colors shadow-2xs cursor-pointer"
            >
              <Mail className="w-3.5 h-3.5 text-red-600" />
              <span>Enviar por Email (Gmail API)</span>
            </button>
      ...
      {/* Modal de Envío de Reporte vía Gmail API */}
      {modalEmailAbierto && (
          ...
          <div className="bg-white rounded-2xl max-w-xl w-full border border-slate-200 shadow-2xl overflow-hidden my-6">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-sm font-bold text-white leading-tight">
                    Enviar Reporte vía Gmail API
                  </h3>
                  <p className="text-[11px] text-slate-400">
                    Notificación ejecutiva oficial a las gerencias registradas
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5">
                ...
                <div className="text-[11px] font-mono text-slate-500 mt-0.5">
                  Código: {proyecto.codigoFiscalSAR || proyecto.codigoPrograma || proyecto.id} • Sección: {proyecto.seccion || 'A'} • Ingreso Estimado: {formatearMoneda(proyecto.ingresoRealTotal || 0, moneda)}
                </div>
              </div>

              {/* Selección de Gerencias Destinatarias */}
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
                  Gerencias Destinatarias Registradas
                </label>
                <div className="space-y-2">
                  {Object.entries(CREDENCIALES_GERENCIAS).map(([clave, gerencia]) => {
                    const seleccionado = destinatariosEmail.includes(gerencia.correo);
                    return (
                      <label
                        key={clave}
                      ...
                      >
                        <input
                          ...
                          onChange={() => handleToggleDestinatario(gerencia.correo)}
                          className="mt-0.5 w-4 h-4 text-red-600 rounded border-slate-300 focus:ring-red-500"
                        />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-bold text-slate-900">{gerencia.nombreGerencia}</span>
                            <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-slate-100 text-slate-600 border border-slate-200">
                              {gerencia.lider}
                            </span>
                          </div>
                          <div className="text-[11px] font-mono text-red-700 truncate mt-0.5">
                            {gerencia.correo}
                          </div>
                        </div>
                      </label>
                ...
                {destinatariosEmail.length === 0 && (
                  <p className="text-[11px] text-rose-600 font-semibold mt-1">
                    ⚠️ Debe seleccionar al menos un correo destinatario.
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                  Nota o Mensaje Institucional Adjunto (Opcional)
                </label>
                <textarea
                  value={notaEmail}
                  onChange={(e) => setNotaEmail(e.target.value)}
                  placeholder="Ej: Se remite reporte financiero y de matrícula para aprobación de presupuesto y asignación docente..."
                  rows={3}
                  className="w-full text-xs p-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none text-slate-800 placeholder-slate-400"
                />
              </div>
            ...
            <div className="bg-slate-50 px-6 py-3.5 border-t border-slate-200 flex items-center justify-between gap-3">
              <span className="text-[11px] text-slate-500">
                Usa API de Gmail con OAuth2 seguro (gapi)
              </span>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                ...
                >
                  {resultadoEnvioEmail?.exito ? 'Cerrar' : 'Cancelar'}
                </button>
                <button
                  id="btn-confirmar-envio-gmail"
                  type="button"
                  disabled={enviandoEmail || destinatariosEmail.length === 0}
                  onClick={handleEnviarEmail}
                  className="flex items-center gap-2 px-4 py-1.5 text-xs font-bold text-white bg-red-600 hover:bg-red-700 disabled:bg-slate-300 disabled:cursor-not-allowed rounded-lg shadow-sm transition-all cursor-pointer"
                >
                  {enviandoEmail ? (
                    <>
                      <Loader2 className="w-3.5 h-3.5 animate-spin" />
                      <span>Autenticando & Enviando...</span>
                    </>
                  ) : (
                    <>
      ...
      {/* Modal Google Sheets API Real-Time Integration */}
      {modalSheetsAbierto && (
        <GoogleSheetsModal
      ...
      {/* Modal Google Forms API Integration */}
      {modalFormsAbierto && (
        <GoogleFormsModal
          isOpen={modalFormsAbierto}
          onClose={() => setModalFormsAbierto(false)}
          proyecto={proyecto}
          proyectos={[proyecto]}
          moneda={moneda}
        />
